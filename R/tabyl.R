...
#'   A 3-way tabulation returns a list of data frames.For 4+ variables, a tabyl_nway object 
#'   (data.frame with class "tabyl_nway") is returned.
#' @export
#' @examples
#'
#' tabyl(mtcars, cyl)
...
tabyl.data.frame <- function(dat, ..., show_na = TRUE, show_missing_levels = TRUE) {
  ...
  # Catch invalid usage like tabyl(mtcars, var2 = am)
  dots <- rlang::enquos(...)
  named <- names(dots)
  if (any(named != "")) {
    stop("please specify var1 OR var1 & var2 OR var1 & var2 & var3", call. = FALSE)
  }
  
  vars <- rlang::ensyms(...)
  
  if (length(vars) == 0) {
    stop("if calling on a data.frame, specify unquoted column name(s) to tabulate. Did you mean to call tabyl() on a vector?")
  }
  
  if (length(vars) == 1) {
    return(tabyl_1way(dat, vars[[1]], show_na = show_na, show_missing_levels = show_missing_levels))
  } else if (length(vars) == 2) {
    return(tabyl_2way(dat, vars[[1]], vars[[2]], show_na = show_na, show_missing_levels = show_missing_levels))
  } else if (length(vars) == 3) {
    return(tabyl_3way(dat, vars[[1]], vars[[2]], vars[[3]], show_na = show_na, show_missing_levels = show_missing_levels))
  } else {
    return(tabyl_nway(dat, !!!vars, show_na = show_na, show_missing_levels = show_missing_levels))
  }
}
